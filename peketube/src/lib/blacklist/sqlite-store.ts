...
function dbFilePath(): string {
  // PEKETUBE_SERVER_DB_PATH es la canónica.
  // KIDSTUBE_SERVER_DB_PATH y BLACKLIST_DB_PATH se aceptan como alias deprecados
  // para no romper despliegues previos al rename a PekeTube.
  const fromEnv =
    process.env.PEKETUBE_SERVER_DB_PATH?.trim() ||
    process.env.KIDSTUBE_SERVER_DB_PATH?.trim() ||
    process.env.BLACKLIST_DB_PATH?.trim();
  if (fromEnv) return fromEnv;
  return path.join(process.cwd(), "data", "peketube.sqlite");
}
...
export function wipePeketubeUserServerStores(userId: string): void {
